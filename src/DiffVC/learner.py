...
class DiffVCLearner:
  ...
  ## loss 계산 ( time step (t)에서의 predicted noise 와 실제 noise )
  def train_step(self, features):
    ...
    with self.autocast:
      ...
      noise_scale = self.noise_level[t].unsqueeze(1).unsqueeze(1)
      noise_scale_sqrt = noise_scale**0.5
      noise = torch.randn_like(spectrogram)
      noisy_spectrogram = noise_scale_sqrt * spectrogram + (1.0 - noise_scale)**0.5 * noise

      predicted = self.model(noisy_spectrogram, t, spectrogram, audio)
...
def _train_impl(replica_id, model, dataset, args, params):
  ...
  learner = DiffVCLearner(args.model_dir, model, dataset, opt, params, fp16=args.fp16)
